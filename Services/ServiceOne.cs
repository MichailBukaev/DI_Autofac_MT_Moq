...
namespace Services
{
    public class ServiceOne : IServiceOne
    {
        ...
        public async Task<string> SendSms(string text)
        {
            ...
            var (smsIsSendResponse, smsIsNotSendResponse) = await client.GetResponse<SMSIsSendMessage, SMSIsNotSendMessage>(new {Text = text});
            ...
            else
            {
                ...
                _logger.LogError("SMS: {@text} is not send with problem {@problem}!", respons.Message.Text, respons.Message.Problem);
